...
import { GetOneMenuRequest,EditMenuRequest, EditNoMenuRequest } from '../../Redux-saga/Action/MenuActions';
...
export default function MenuAdd(props) {
    ...
    const { menu } = useSelector(state => state.menuStated)
    const [previewImg, setPreviewImg] = useState();
    const [uploaded, setUploaded] = useState(false);


    useEffect(() => {
        dispatch(GetOneMenuRequest(props.id))
    }, [])

    useEffect(() => {
        let img = `${config.domain + '/menu/file/' + menu.menu_image}`
        setPreviewImg(img)
    }, [menu])
    ...
    const formik = useFormik({
        ...
        initialValues: {
            ...
            price: menu.price,
            menu_imagw: config.domain+'/menu/file/'+menu.menu_image
        },
        validationSchema : Yup.object({
            // menu_id: Yup.string("Enter menu ID").required("menu ID is required"),
            cate_id: Yup.string("Enter Category ID").required("Category is required"),
            menu_name: Yup.string("Enter Name").required("Name is required"),
        ...
        onSubmit: async (values) => {
            if (uploaded === true) {
                ...
                payload.append('menu_id', menu.menu_id)
                dispatch(EditMenuRequest(payload))
            } else {
                const payload = {
                ...
                dispatch(EditNoMenuRequest(payload))
            }
            props.closeAdd();
